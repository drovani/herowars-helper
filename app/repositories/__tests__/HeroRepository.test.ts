...
describe('HeroRepository', () => {
  ...
  describe('Equipment Relationships', () => {
    it('should find heroes using specific equipment', async () => {
      ...
      const result = await repository.findHeroesUsingEquipment('test-equipment')
    ...
    it('should get hero equipment by quality', async () => {
      ...
      const result = await repository.getHeroEquipmentByQuality('test-hero', 'white')
  ...
  describe('Bulk Operations', () => {
    ...
    it('should create hero with all related data', async () => {
      ...
      const mockBulkCreateArtifacts = vi.fn().mockResolvedValue({
        data: [{ id: 'artifact-1', ...heroWithData.artifacts![0], created_at: new Date().toISOString() }],
        error: null,
      })
      const mockBulkCreateSkins = vi.fn().mockResolvedValue({
        data: [{ id: 'skin-1', ...heroWithData.skins![0], created_at: new Date().toISOString() }],
        error: null,
      })
      const mockBulkCreateGlyphs = vi.fn().mockResolvedValue({
        data: [{ id: 'glyph-1', ...heroWithData.glyphs![0], created_at: new Date().toISOString() }],
        error: null,
      })
      const mockBulkCreateEquipmentSlots = vi.fn().mockResolvedValue({
        data: [{ id: 'slot-1', ...heroWithData.equipmentSlots![0], created_at: new Date().toISOString() }],
        error: null,
      })
      ...
      expect(mockBulkCreateArtifacts).toHaveBeenCalledWith(heroWithData.artifacts)
      ...
      expect(mockBulkCreateEquipmentSlots).toHaveBeenCalledWith(heroWithData.equipmentSlots)
  ...
  describe('Error Handling', () => {
    ...
    it('should handle bulk operation errors gracefully', async () => {
      ...
      expect(result.error?.message).toContain('Bulk artifact creation completed with 1 errors')
