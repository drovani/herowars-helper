...
describe('Admin Setup - Equipment Import', () => {
  ...
  describe('Equipment dataset import', () => {
    it('should successfully import equipment data', async () => {
      ...
      const mockInitResult = {
        data: {
          ...
          stats: [{ equipment_slug: 'test-sword', stat: 'strength', value: 100 }],
          required_items: [{ base_slug: 'test-sword', required_slug: 'component-1', quantity: 2 }],
        },
        error: null,
      }

      vi.spyOn(EquipmentRepository.prototype, 'initializeFromJSON').mockResolvedValue(mockInitResult)
      ...
      expect(EquipmentRepository.prototype.initializeFromJSON).toHaveBeenCalledWith([
        expect.objectContaining({ slug: 'test-sword', name: 'Test Sword' }),
        expect.objectContaining({ slug: 'test-fragment', name: 'Test Fragment (Fragment)' }),
      ])
    })
    ...
    it('should handle equipment import errors', async () => {
      ...
      vi.spyOn(EquipmentRepository.prototype, 'initializeFromJSON').mockResolvedValue({
      ...
      expect(data.results.equipment.errorDetails[0].record.slug).toBe('test-sword')
      expect(data.results.equipment.skippedDetails[0].slug).toBe('test-fragment')
    })
    ...
    it('should include equipment in "all" dataset mode', async () => {
      ...
      expect(EquipmentRepository.prototype.initializeFromJSON).toHaveBeenCalled()
    })
    ...
    it('should skip equipment when dataset is "missions"', async () => {
      ...
      const equipmentSpy = vi.spyOn(EquipmentRepository.prototype, 'initializeFromJSON')
  ...
  describe('Equipment data transformation', () => {
    it('should correctly transform equipment JSON data', async () => {
      ...
      vi.spyOn(EquipmentRepository.prototype, 'initializeFromJSON').mockImplementation((data) => {
  ...
  describe('Logging', () => {
    it('should call equipment repository initialization when dataset is equipment', async () => {
      ...
      const initSpy = vi.spyOn(EquipmentRepository.prototype, 'initializeFromJSON').mockResolvedValue({
        data: { equipment: [], stats: [], required_items: [] },
        error: null,
      })
      ...
      expect(initSpy).toHaveBeenCalledWith(expect.arrayContaining([
