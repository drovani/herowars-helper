...
describe("useQueryState", () => {
  ...
  it("returns query parameter value when it exists", () => {
    const { result } = renderHook(() => useQueryState("status", "default"), {
      wrapper: ({ children }) => (
        <TestWrapper initialEntries={["/?status=active"]}>
          {children}
        </TestWrapper>
      ),
    });
  ...
  it("preserves other query parameters when updating", () => {
    const { result: result1 } = renderHook(
      ...
      {
        wrapper: ({ children }) => (
          <TestWrapper initialEntries={["/?existing=value&filter=active"]}>
            {children}
          </TestWrapper>
        ),
      }
  ...
  it("handles empty string values", () => {
    const { result } = renderHook(() => useQueryState("search", ""), {
      wrapper: ({ children }) => (
        <TestWrapper initialEntries={["/?search="]}>{children}</TestWrapper>
      ),
    });
  ...
  it("works with complex query parameter names", () => {
    const { result } = renderHook(
      ...
      {
        wrapper: ({ children }) => (
          <TestWrapper initialEntries={["/?sort_by=date"]}>
            {children}
          </TestWrapper>
        ),
      }
