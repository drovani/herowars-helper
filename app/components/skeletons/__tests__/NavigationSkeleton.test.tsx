...
describe("NavigationSkeleton", () => {
  ...
  describe("sidebar type (default)", () => {
    it("should render sidebar skeleton by default", () => {
      ...
      const userAvatar = result.container.querySelector(
        '[class*="size-10"][class*="rounded-full"]'
      );
      expect(userAvatar).toBeInTheDocument();

      // Should have user info skeletons
      const userNameSkeleton = result.container.querySelector(
        '[class*="h-4"][class*="w-24"]'
      );
      const userRoleSkeleton = result.container.querySelector(
        '[class*="h-3"][class*="w-16"]'
      );
    ...
    it("should render menu sections", () => {
      const result = render(
        <NavigationSkeleton type="sidebar" itemCount={6} />
      );

      // Should have section headers
      const sectionHeaders = result.container.querySelectorAll(
        '[class*="h-5"][class*="w-32"]'
      );
      expect(sectionHeaders).toHaveLength(2); // 2 menu sections

      // Should have menu items (6 items split across 2 sections = 3 per section)
      const menuItems = result.container.querySelectorAll(
        '[class*="h-4"][class*="w-20"]'
      );
    ...
    it("should render bottom section", () => {
      ...
      const bottomSection = result.container.querySelector(
        '[class*="pt-4"][class*="border-t"]'
      );
      expect(bottomSection).toBeInTheDocument();

      // Should have bottom menu item
      const bottomMenuItem = bottomSection?.querySelector(
        '[class*="h-4"][class*="w-16"]'
      );
    ...
    it("should handle custom item count", () => {
      const result = render(
        <NavigationSkeleton type="sidebar" itemCount={8} />
      );
  ...
  describe("header type", () => {
    ...
    it("should render left side of header", () => {
      ...
      const brandText = result.container.querySelector(
    ...
    it("should render right side of header", () => {
      ...
      const userButton = result.container.querySelector(
        '[class*="h-8"][class*="w-24"]'
      );
      const userAvatar = result.container.querySelector(
        '[class*="size-8"][class*="rounded-full"]'
      );
    ...
    it("should use proper flex layout", () => {
      ...
      const flexSections = result.container.querySelectorAll(
  ...
  describe("breadcrumbs type", () => {
    ...
    it("should render breadcrumb items with separators", () => {
      ...
      const breadcrumbItems = result.container.querySelectorAll(
  ...
  describe("skeleton elements", () => {
    ...
    it("should render skeleton elements with various sizes", () => {
      ...
      const smallElements = result.container.querySelectorAll(
        '[class*="h-3"], [class*="h-4"]'
      );
      const mediumElements = result.container.querySelectorAll(
        '[class*="h-5"], [class*="h-6"]'
      );
      const avatarElements =
        result.container.querySelectorAll('[class*="size-"]');
    ...
    it("should use appropriate widths for different elements", () => {
      ...
      const narrowElements = result.container.querySelectorAll(
        '[class*="w-12"], [class*="w-16"]'
      );
      const mediumElements = result.container.querySelectorAll(
        '[class*="w-20"], [class*="w-24"]'
      );
  ...
  describe("responsive behavior", () => {
    it("should maintain proper structure across different types", () => {
      ...
      const breadcrumbsResult = render(
  ...
  describe("edge cases", () => {
    it("should handle zero item count", () => {
      const result = render(
        <NavigationSkeleton type="sidebar" itemCount={0} />
      );
    ...
    it("should handle large item count", () => {
      const result = render(
        <NavigationSkeleton type="sidebar" itemCount={20} />
      );
    ...
    it("should handle odd item counts properly", () => {
      const result = render(
        <NavigationSkeleton type="sidebar" itemCount={7} />
      );
  ...
  describe("accessibility", () => {
    ...
    it("should handle different navigation types accessibly", () => {
      const types: Array<"sidebar" | "header" | "breadcrumbs"> = [
        "sidebar",
        "header",
        "breadcrumbs",
      ];
