...
export class PlayerTeamRepository extends BaseRepository<'player_team'> {
   ...
   * Update team details
   */
  async updateTeam(teamId: string, userId: string, updates: UpdatePlayerTeamInput): Promise<RepositoryResult<PlayerTeam>> {
    try {
      const { data, error } = await this.supabase
        .from('player_team')
        .update(updates)
   ...
   */
  async addHeroToTeam(teamId: string, userId: string, heroData: AddHeroToTeamInput): Promise<RepositoryResult<PlayerTeamHero>> {
    try {
      ...
      log.info(`Added hero ${heroData.hero_slug} to team ${teamId}`)
      return { data, error: null }
    } catch (err) {
