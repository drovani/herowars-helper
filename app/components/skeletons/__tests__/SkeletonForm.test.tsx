...
describe("SkeletonForm", () => {
  ...
  describe("form structure", () => {
    it("should render with default 4 fields and 1 section", () => {
      ...
      const fieldGroups = result.container.querySelectorAll(
        'div[class*="space-y-2"]'
      );
      ...
      fieldGroups.forEach((fieldGroup) => {
        const skeletons = fieldGroup.querySelectorAll(
          '[class*="animate-pulse"]'
        );
    ...
    it("should render with multiple sections", () => {
      ...
      const sectionHeaders = result.container.querySelectorAll(
        'div[class*="space-y-4"] [class*="h-6"][class*="w-32"]'
      );
  ...
  describe("header configuration", () => {
    it("should show header by default", () => {
      ...
      const cardHeader =
        result.container.querySelector('[class*="CardHeader"]') ||
        result.container.querySelector("div")?.children[0];
      expect(cardHeader).toBeInTheDocument();

      // Should have header skeletons (title + description)
      const headerSkeletons = result.container.querySelectorAll(
        '[class*="h-6"][class*="w-48"], [class*="h-4"][class*="w-64"]'
      );
    ...
    it("should show header when explicitly enabled", () => {
      ...
      const headerTitle = result.container.querySelector(
        '[class*="h-6"][class*="w-48"]'
      );
  ...
  describe("buttons configuration", () => {
    it("should show buttons by default", () => {
      ...
      const buttonSkeletons = result.container.querySelectorAll(
        '[class*="h-10"][class*="w-24"], [class*="h-10"][class*="w-20"]'
      );
    ...
    it("should hide buttons when disabled", () => {
      ...
      const buttonContainer = result.container.querySelector(
        'div[class*="flex"][class*="gap-2"][class*="pt-4"]'
      );
    ...
    it("should show buttons when explicitly enabled", () => {
      ...
      const buttonSkeletons =
        buttonContainer?.querySelectorAll('[class*="h-10"]');
      expect(buttonSkeletons?.length).toBeGreaterThanOrEqual(2);
    });
  });
  ...
  describe("field structure", () => {
    it("should render proper field structure", () => {
      ...
      fieldGroups.forEach((fieldGroup) => {
        const labelSkeleton = fieldGroup.querySelector(
          '[class*="h-4"][class*="w-24"]'
        );
        const inputSkeleton = fieldGroup.querySelector(
          '[class*="h-10"][class*="w-full"]'
        );
  ...
  describe("skeleton elements", () => {
    ...
    it("should render full-width input skeletons", () => {
      ...
      const fullWidthInputs = result.container.querySelectorAll(
        '[class*="h-10"][class*="w-full"]'
      );
  ...
  describe("combined configurations", () => {
    ...
    it("should handle header-only configuration", () => {
      const result = render(
        <SkeletonForm fields={0} showHeader={true} showButtons={false} />
      );
  ...
  describe("edge cases", () => {
    ...
    it("should maintain proper spacing", () => {
      ...
      const spacingElements = result.container.querySelectorAll(
        '[class*="space-y-"]'
      );
  ...
  describe("responsiveness", () => {
    it("should maintain proper structure across screen sizes", () => {
      ...
      const fullWidthInputs =
        result.container.querySelectorAll('[class*="w-full"]');
      expect(fullWidthInputs.length).toBeGreaterThan(0);
    });
  });
});
