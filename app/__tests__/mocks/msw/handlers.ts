...
const validateSupabaseHeaders = (request: Request) => {
  ...
  if (!apiKey || !authorization) {
    return HttpResponse.json(SUPABASE_ERRORS.PERMISSION_DENIED.error, {
      status: 401,
    });
  }
  return null;
...
const equipmentHandlers = [
  ...
  http.get("*/rest/v1/equipment", ({ request }) => {
    ...
    if (filter && filter.startsWith("eq.")) {
      ...
      const equipment =
        equipmentStore.find((e) => e.slug === slug) ||
        createMockEquipment({ slug });
    ...
    let data =
      equipmentStore.length > 0 ? equipmentStore : createMockEquipmentList(3);
  ...
  http.post("*/rest/v1/equipment", async ({ request }) => {
    ...
    if (existing) {
      return HttpResponse.json(SUPABASE_ERRORS.UNIQUE_CONSTRAINT.error, {
        status: 409,
      });
    }
  ...
  http.patch("*/rest/v1/equipment", async ({ request }) => {
    ...
    if (!filter || !filter.startsWith("eq.")) {
      return HttpResponse.json(SUPABASE_ERRORS.VALIDATION_ERROR.error, {
        status: 400,
      });
    }
    ...
    if (equipmentIndex === -1) {
      return HttpResponse.json(SUPABASE_ERRORS.NOT_FOUND.error, {
        status: 404,
      });
    }
...
const missionHandlers = [
  ...
  http.get("*/rest/v1/mission", ({ request }) => {
    ...
    let data: Mission[] =
      missionStore.length > 0 ? missionStore : [createMockMission()];
...
const chapterHandlers = [
  ...
  http.get("*/rest/v1/chapter", ({ request }) => {
    ...
    let data: Chapter[] =
      chapterStore.length > 0 ? chapterStore : [createMockChapter()];
...
const errorHandlers = [
  ...
  http.get("*/rest/v1/network-error", () => {
    return HttpResponse.json(SUPABASE_ERRORS.NETWORK_ERROR.error, {
      status: 500,
    });
  }),
