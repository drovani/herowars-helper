...
export function HeroIndexSkeleton({ 
  ...
  const cardIndices = useMemo(() => Array.from({ length: cappedItemCount }, (_, i) => i), [cappedItemCount]);
  const tileIndices = useMemo(() => Array.from({ length: tileItemCount }, (_, i) => i), [tileItemCount]);
  if (mode === "cards") {
    return (
      <div className={cn("space-y-4", className)} role="status" aria-live="polite" aria-label="Loading hero list">
        {/* Search and toggle controls */}
        <div className="flex justify-between gap-4" aria-label="Loading hero search controls">
          <Skeleton className="h-10 w-full max-w-sm" aria-label="Loading search input" />
          <div className="hidden md:flex gap-1" aria-label="Loading view mode toggles">
            <Skeleton className="h-10 w-10" aria-label="Loading cards view toggle" />
            <Skeleton className="h-10 w-10" aria-label="Loading tiles view toggle" />
          </div>
        </div>

        {/* Hero cards grid */}
        <div className="gap-2 grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5" role="grid" aria-label={`Loading ${cappedItemCount} heroes`}>
          {cardIndices.map((index) => (
            <div key={index} className="relative group size-28" role="gridcell" aria-label={`Loading hero ${index + 1}`}>
  ...
  // Tiles mode
  return (
    <div className={cn("space-y-4", className)} role="status" aria-live="polite" aria-label="Loading hero tiles">
      {/* Search and toggle controls */}
      <div className="flex justify-between gap-4" aria-label="Loading hero search controls">
        <Skeleton className="h-10 w-full max-w-sm" aria-label="Loading search input" />
        <div className="hidden md:flex gap-1" aria-label="Loading view mode toggles">
      ...
      {/* Tiles header */}
      <div className="grid grid-cols-5 text-center font-medium sticky" role="rowgroup" aria-label="Loading table headers">
        <div>Hero</div>
      ...
      {/* Hero tiles */}
      <div className="flex flex-col gap-4" role="table" aria-label={`Loading ${tileItemCount} hero details`}>
        {tileIndices.map((index) => (
          <Card key={index} className="w-full grid grid-cols-2 md:grid-cols-5" role="row" aria-label={`Loading hero details ${index + 1}`}>
            {/* Hero column */}
      ...
      <div className="flex justify-center gap-2 mt-6" aria-label="Loading pagination controls">
        <Skeleton className="h-9 w-20" aria-label="Loading previous page button" />
