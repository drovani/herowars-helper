 ...
 */
export const createMockEquipmentList = (count: number, baseOverrides: Partial<Equipment> = {}): Equipment[] => {
...
export const createMockMissionList = (count: number, baseOverrides: Partial<Mission> = {}): Mission[] => {
  return Array.from({ length: count }, (_, index) =>
    createMockMission({
      ...baseOverrides,
...
export const createMockChapterList = (count: number, baseOverrides: Partial<Chapter> = {}): Chapter[] => {
  return Array.from({ length: count }, (_, index) =>
    createMockChapter({
      ...baseOverrides,
      id: index + 1,
 ...
 */
export const createSupabaseError = (code: string, message: string, details?: string) => ({
  error: {
    code,
    message,
...
export const SUPABASE_ERRORS = {
  NOT_FOUND: createSupabaseError('PGRST116', 'Record not found', 'No rows found'),
  UNIQUE_CONSTRAINT: createSupabaseError('23505', 'Unique constraint violation', 'Key already exists'),
  PERMISSION_DENIED: createSupabaseError('42501', 'Permission denied', 'RLS violation'),
  NETWORK_ERROR: createSupabaseError('500', 'Internal server error', 'Database connection failed'),
  VALIDATION_ERROR: createSupabaseError('422', 'Validation error', 'Invalid input data'),
}