...
async function testHeroRepository() {
  try {
    ...
    const migrationResult = transformHeroData(sampleHeroes, {
    ...
    console.log(
      `   - Equipment Slots: ${migrationResult.equipmentSlots.length}`
    );
    ...
    if (allHeroes.data?.length === 0) {
      ...
      if (allHeroes.data && allHeroes.data.length > 0) {
        ...
        if (heroWithData.error) {
          console.log(
            `❌ Complex hero query failed: ${heroWithData.error.message}`
          );
        } else {
          console.log(
            `✅ Complex hero query successful for: ${heroWithData.data?.name}`
          );
          console.log(
            `   - Artifacts: ${heroWithData.data?.artifacts?.length || 0}`
          );
          console.log(`   - Skins: ${heroWithData.data?.skins?.length || 0}`);
          console.log(`   - Glyphs: ${heroWithData.data?.glyphs?.length || 0}`);
          console.log(
            `   - Equipment Slots: ${
              heroWithData.data?.equipmentSlots?.length || 0
            }`
          );
        }
      }
