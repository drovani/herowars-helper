...
CREATE TYPE "public"."equipment_quality" AS ENUM (
    'gray',
    'green',
    'blue',
    'violet',
...
CREATE OR REPLACE FUNCTION "public"."get_equipment_with_neighbors"("slug_input" "text") RETURNS TABLE("quality" "text", "slug" "text", "name" "text")
    LANGUAGE "plpgsql"
...
BEGIN
    RETURN QUERY
    WITH RankedRows AS (
        SELECT equipment.quality::text, equipment.slug, equipment.name,
               LAG(equipment.slug) OVER (ORDER BY equipment.quality, equipment.slug) AS previous_slug,
               LEAD(equipment.slug) OVER (ORDER BY equipment.quality, equipment.slug) AS next_slug
        FROM equipment
    )
...
CREATE OR REPLACE FUNCTION "public"."handle_new_user"() RETURNS "trigger"
    ...
    AS $$
BEGIN
  INSERT INTO public.user_roles (id, roles)
  VALUES (NEW.id, '{"user"}'); -- Default role for new users
  RETURN NEW;
END;
$$;
...
CREATE TABLE IF NOT EXISTS "public"."equipment_required_item" (
...
CREATE TABLE IF NOT EXISTS "public"."equipment_stat" (
...
CREATE POLICY "Enable all access for all users" ON "public"."chapter" USING (true);



CREATE POLICY "Enable all access for all users" ON "public"."equipment" USING (true);



CREATE POLICY "Enable all access for all users" ON "public"."equipment_required_item" USING (true);



CREATE POLICY "Enable all access for all users" ON "public"."equipment_stat" USING (true);



CREATE POLICY "Enable all access for all users" ON "public"."mission" USING (true);
...
CREATE POLICY "Enable read access for all users" ON "public"."equipment_required_item" FOR SELECT USING (true);



CREATE POLICY "Enable read access for all users" ON "public"."equipment_stat" FOR SELECT USING (true);
