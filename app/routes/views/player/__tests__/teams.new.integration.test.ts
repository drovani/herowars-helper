...
describe('Player Teams New Integration', () => {
  ...
  describe('loader', () => {
    it('should load user hero collection for team building', async () => {
      ...
      mockPlayerHeroRepo.findWithHeroDetails.mockResolvedValue({ data: mockUserHeroes, error: null })
      ...
      expect(mockPlayerHeroRepo.findWithHeroDetails).toHaveBeenCalledWith('user1')
    })
    ...
    it('should return empty hero collection when user has no heroes', async () => {
      mockPlayerHeroRepo.findWithHeroDetails.mockResolvedValue({ data: [], error: null })
  ...
  describe('action - createTeam', () => {
    it('should create team with heroes successfully', async () => {
      ...
      const actionRequest = new Request('http://localhost:3000/player/teams/new', {
        method: 'POST',
        body: formData
      })
      ...
      expect(mockTeamRepo.addHeroToTeam).toHaveBeenNthCalledWith(1, 'new-team-id', 'user1', { hero_slug: 'astaroth' })
      expect(mockTeamRepo.addHeroToTeam).toHaveBeenNthCalledWith(2, 'new-team-id', 'user1', { hero_slug: 'aurora' })
      expect(mockTeamRepo.addHeroToTeam).toHaveBeenNthCalledWith(3, 'new-team-id', 'user1', { hero_slug: 'celeste' })
    })
    ...
    it('should clean up team and return error when adding hero fails', async () => {
      ...
      mockTeamRepo.addHeroToTeam
        ...
        .mockResolvedValueOnce({ data: null, error: new Error('Hero not found') }) // Second hero fails
      ...
      expect(result.error).toBe('Failed to add hero invalid-hero: Hero not found')
      expect(mockTeamRepo.deleteTeam).toHaveBeenCalledWith('new-team-id', 'user1')
    })
  })
