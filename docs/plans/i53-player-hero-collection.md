...
- Update `app/repositories/types.ts` with new PlayerHero types

### 1.3 Create PlayerHeroRepository
...
- Implement methods:
  ...
  - `addHeroToCollection(userId: string, heroSlug: string)` - Add hero
  - `updateHeroProgress(userId: string, heroSlug: string, updates)` - Update progression
  - `removeFromCollection(userId: string, heroSlug: string)` - Remove hero
