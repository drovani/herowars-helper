...
describe('hero-data-migration', () => {
  ...
  describe('transformHeroData', () => {
    ...
    it('should handle heroes with partial glyphs array', () => {
      const heroWithPartialGlyphs: HeroRecord = {
        ...sampleJsonHero,
        glyphs: ['physical attack', null, 'health', undefined, 'strength'] as any,
      }
    ...
    it('should skip invalid data when skipInvalidData is true', () => {
      ...
      const result = transformHeroData([sampleJsonHero, invalidHero], { skipInvalidData: true })
  ...
  describe('validateMigrationResult', () => {
    ...
    it('should detect duplicate hero slugs', () => {
      ...
      const hero2 = { ...sampleJsonHero, slug: 'duplicate', name: 'Different Name' }
      
      const result = transformHeroData([hero1, hero2])
      const validationErrors = validateMigrationResult(result)

      expect(validationErrors).toContain('Duplicate hero slugs found: duplicate')
    })
    ...
    it('should detect orphaned artifacts', () => {
      ...
      expect(validationErrors).toContain('Found 1 artifacts with missing hero references')
    })
    ...
    it('should detect invalid glyph positions', () => {
      ...
      expect(validationErrors).toContain('Found 1 glyphs with invalid positions (must be 1-5)')
    })
    ...
    it('should detect invalid equipment slot positions', () => {
      ...
      expect(validationErrors).toContain('Found 1 equipment slots with invalid positions (must be 1-6)')
    })
  })
